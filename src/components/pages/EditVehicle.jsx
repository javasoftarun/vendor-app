...
export default function EditVehicle() {
  ...
  const [form, setForm] = useState(location.state?.vehicle || null);
  ...
  useEffect(() => {
    if (!form) {
      alert("Vehicle details not found. Please go back and try again.");
      navigate(-1);
    }
  ...
  // Fetch lat/lng if missing and address is present
  useEffect(() => {
    ...
    if (
      form &&
      form.address &&
      (!form.latitude || !form.longitude || form.latitude === "null" || form.longitude === "null")
    ) {
      fetchLatLng(form.address);
    }
    // eslint-disable-next-line
  }, [form && form.address]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(API_ENDPOINTS.UPDATE_VEHICLE_BY_REG_ID(form.registrationId), {
        method: "PUT",
      ...
      if (response.ok && data.responseCode === 200) {
        ...
        setModal({
          open: true,
          message: data.responseMessage || "Failed to update vehicle. Please try again.",
          success: false
        });
      }
    } catch (error) {
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              Please update the information below and click "Save Changes" to update your vehicle.
            </div>
          </div>
